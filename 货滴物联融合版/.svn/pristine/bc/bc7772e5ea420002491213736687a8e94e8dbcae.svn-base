...
+ (void)zhiFuBaoname: (NSString*)name titile:(NSString*)title price:(NSString*)price orderId:(NSString*)orderId notice:(NSString *)noticeID
{
    ...
    order.biz_content.subject = [NSString stringWithFormat:@"货滴物联充值%.2f元",[price floatValue]];
    ...
    if (signedString != nil) {
        ...
        NSString *appScheme = @"huodiwulianScheme";
...
#pragma mark - - - - - - 预支付签名
    ...
    return [[Paydetail1 md5:signstr] uppercaseString];
}
...
+ (void)prepayIdRequestname:(NSString*)name notifyurl:(NSString*)notifyurl outTradeno:(NSString*)orderno1 orderMoney:(NSString*)orderMoney1
{
    ...
    NSString* nonce_str = [Paydetail1 ret32bitString];// @"5K8264ILTKCH16CQ2502SI8ZNMTM67VS";//;//
    ...
    //NSString* spbill_create_ip = [Paydetail1 getIPAddress];
    NSString* spbill_create_ip = [Paydetail1 getDeviceIPAddresses];
    ...
    NSString* sign = [Paydetail1 creatsign:parmaArray];//@"C380BEC2BFD727A4B6845133519F3AD6";
    NSLog(@"sign = %@",sign);
    ...
    if ([[dict objectForKey:@"return_code"]isEqualToString:@"SUCCESS"]) {//预处理成功
        if ([[dict objectForKey:@"result_code"]isEqualToString:@"SUCCESS"]) {
            ...
            req.sign= [Paydetail1 creatPaysign:array];
