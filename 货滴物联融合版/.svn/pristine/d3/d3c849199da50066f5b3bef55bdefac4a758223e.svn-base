...
#import "CancleOrderView1.h"

@implementation CancleOrderView1

+ (instancetype)headerViewWithTableView:(UITableView *)tableView{
    static NSString *headerID = @"CancleOrderView1";
    CancleOrderView1 *ancleOrderView1 = [tableView dequeueReusableHeaderFooterViewWithIdentifier:headerID];
    if (ancleOrderView1 == nil) {
        
        ancleOrderView1 = [[CancleOrderView1 alloc]initWithReuseIdentifier:headerID];
        
    }
    
    return ancleOrderView1;
}
-(void)setStatus:(NSArray *)status{
    _status = status;
    if ([[NSString stringWithFormat:@"%@",_status[0]] isEqualToString:@"0"]&&[[NSString stringWithFormat:@"%@",_status[1]] isEqualToString:@"0"]) {//等待接货 显示开始按钮
        _startbut.hidden = NO;
        ...
        _endbut.hidden = YES;
    }else if ([[NSString stringWithFormat:@"%@",_status[0]] isEqualToString:@"0"]&&[[NSString stringWithFormat:@"%@",_status[1]] isEqualToString:@"-2"]){//等待接货 不显示开始
        _startbut.hidden = YES;
        _driverSybut.hidden = YES;
        _canclebut.hidden = NO;
        _endbut.hidden = YES;
        _canclebut.frame = CGRectMake(20, 30, SCREEN_WIDTH-40, 50);
