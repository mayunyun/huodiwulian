...
#import "HomeMapViewC11.h"
#import "WuLiuSeaPolTableViewCell1.h"
#import "SYCitySelectionController.h"

@interface HomeMapViewC11 ()<MAMapViewDelegate, AMapLocationManagerDelegate,AMapSearchDelegate,UISearchBarDelegate,UITableViewDelegate,UITableViewDataSource>
{
...
- (void)viewDidLoad {
    ...
    [newBut setTitle:@"确认地址" forState:UIControlStateNormal];
...
- (void)newButClick:(UIButton *)but{
    if ( [[Command convertNull:self.mapStr] isEqualToString:@""]) {
        jxt_showAlertTitle(@"请选择地址");
        return;
    }
    
    if (_qidianBlock) {
        _qidianBlock(self.mapStr,_provinceStr,_cityStr,_areaStr);
    }
...
- (void)configLocationManager
{
    ...
    [self.locationManager requestLocationWithReGeocode:YES completionBlock:^(CLLocation *location, AMapLocationReGeocode *regeocode, NSError *error) {
        ...
        if (regeocode)
        {
            ...
            _provinceStr = regeocode.province;
            _cityStr = regeocode.city;
            _areaStr = regeocode.district;
...
- (void)onReGeocodeSearchDone:(AMapReGeocodeSearchRequest *)request response:(AMapReGeocodeSearchResponse *)response
{
    if (response.regeocode != nil)
    {
        ...
        self.mapStr = result;
        _mapLab.text = result;
        _provinceStr = [NSString stringWithFormat:@"%@", response.regeocode.addressComponent.province];
        _cityStr = [NSString stringWithFormat:@"%@", response.regeocode.addressComponent.city];
        _areaStr = [NSString stringWithFormat:@"%@", response.regeocode.addressComponent.district];
    }
