...
#import "HDSousuoViewController1.h"

@interface HuodongViewController1 ()<UITableViewDataSource,UITableViewDelegate>{
    NSInteger _page;
...
@implementation HuodongViewController1
...
- (UITableView *)tableview{
    if (_tableview == nil) {
        ...
        [_tableview registerClass:[MainTableViewCell1 class] forCellReuseIdentifier:NSStringFromClass([MainTableViewCell1 class])];
...
- (void)loadNew
{
    ...
    NSDictionary* params = @{@"page":[NSString stringWithFormat:@"%li",(long)_page],@"rows":@"8",@"params":[NSString stringWithFormat:@"{\"appstatus\":\"%@\"}",@"1"]};
    NSLog(@"%@",params);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    Class MainClass = [MainTableViewCell1 class];
    MainTableViewCell1 *cell = nil;
    ...
    if (_dataArr.count) {
        HuoDongModel1*model = self.dataArr[indexPath.row];
        cell.model = model;
    }
...
- (void)rightToLastViewController:(UIButton *)button{
    HDSousuoViewController1 *sousuo = [[HDSousuoViewController1 alloc]init];
