//
//  HomeSYBanJDetailsViewController1.m
//  BasicFramework
//
...
@interface HomeSYBanJDetailsViewController1 ()<UIScrollViewDelegate,AMapSearchDelegate>
...
@implementation HomeSYBanJDetailsViewController1
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"搬家单详情";
...
- (void)qiangdanButClick{
    ...
    NSDictionary* params = @{@"params":[NSString stringWithFormat:@"{\"id\":\"%@\",\"ordertype\":\"%@\"}",self.idstr,@"0"]};
    NSLog(@"抢单参数==%@",params);
    [[RequestTool1 sharedRequestTool].UserRequestTool1 loginWithUrl:URLStr Parameters:params FinishedLogin:^(id responseObject) {
...
-(void)fahuodanButClicked:(UIButton *)but{
    [SMAlert hide:NO];
    TongChBanViewController1 *tongchban = [[TongChBanViewController1 alloc]init];
    tongchban.push = @"1";
    [self.navigationController pushViewController:tongchban animated:YES];
