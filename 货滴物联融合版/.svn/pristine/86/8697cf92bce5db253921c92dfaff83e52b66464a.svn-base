...
@interface WLMapViewController1 ()<MAMapViewDelegate, AMapLocationManagerDelegate,AMapSearchDelegate,UISearchBarDelegate,UITableViewDelegate,UITableViewDataSource>
{
...
@property (nonatomic, strong)NSString *mapStraddress;
...
@implementation WLMapViewController1
...
- (void)newButClick:(UIButton *)but{
    ...
    if (_zhongdianBlock) {
        _zhongdianBlock(self.mapStr,self.locationStr,self.mapStraddress);
    }
    if (_qidianBlock) {
        _qidianBlock(self.mapStr,self.locationStr,self.mapStraddress);
    }
...
- (void)onReGeocodeSearchDone:(AMapReGeocodeSearchRequest *)request response:(AMapReGeocodeSearchResponse *)response
{
    if (response.regeocode != nil)
    {
        ...
        self.mapStr = result;
        self.mapStraddress = [NSString stringWithFormat:@"%@", response.regeocode.formattedAddress];
        _mapLab.text = result;
    }
