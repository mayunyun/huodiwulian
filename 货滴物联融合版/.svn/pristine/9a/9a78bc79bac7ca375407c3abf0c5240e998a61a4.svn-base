...
#import "YGDatePicker1.h"

@interface YGDatePicker1 ()
...
@implementation YGDatePicker1

+ (instancetype)sharedDatePicker {
    
    static YGDatePicker1 *datePicker = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!datePicker) {
            datePicker = [YGDatePicker1 new];
        }
    });
    return datePicker;
}
...
{
    [[YGDatePicker1 sharedDatePicker] showDateDetermineChoose:determineChoose];
}
- (void)showDateDetermineChoose:(void(^)(NSString *dateString))determineChoose
{
    _determineChoose = determineChoose;
    YGDatePicker1 *picker = [YGDatePicker1 sharedDatePicker];
