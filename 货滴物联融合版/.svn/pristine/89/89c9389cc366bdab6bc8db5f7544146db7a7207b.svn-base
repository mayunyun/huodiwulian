...
-(void)createPageControl{
    ...
    _pageControl.pageIndicatorTintColor = ThemeColor;
...
- (void)createCycleView {
    ...
    [_cycleCollectionView registerClass:[BannerCollectionViewCell class] forCellWithReuseIdentifier:ZYCycleID];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    BannerCollectionViewCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:ZYCycleID forIndexPath:indexPath];
    ...
    HuoDongModel * model = self.imagesArr[imageIndex];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.delegate respondsToSelector:@selector(ZYCollectionViewClick:strID:)]) {
        self.index = [self indexWithOffset:indexPath.item];
        NSInteger imageIndex       = self.index;
        HuoDongModel * model = self.imagesArr[imageIndex];
        [self.delegate ZYCollectionViewClick:[self indexWithOffset:indexPath.item] strID:model];
    }
}
