...
@interface OrderDaterViewController1 ()<UITableViewDelegate,UITableViewDataSource>
{
...
@property (nonatomic,strong) MYYMyOrderModel1 *Ordermodel;
@end

@implementation OrderDaterViewController1
...
- (void)dataRequest{
    ...
    NSLog(@"%@",params);
    [[RequestTool1 sharedRequestTool].UserRequestTool1 loginWithUrl:@"/mbtwz/scshop?action=getOneOrder" Parameters:params FinishedLogin:^(id responseObject) {

        NSArray *array = [NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableContainers error:nil];
        NSLog(@"%@",array);
        if (!IsEmptyValue(array)) {
            _Ordermodel=[[MYYMyOrderModel1 alloc]init];
...
- (void)butwhihGo:(UIButton *)but{
    ...
    NSLog(@"%@",params);
    [[RequestTool1 sharedRequestTool].UserRequestTool1 loginWithUrl:@"/mbtwz/scshop?action=updateOrder_4" Parameters:params FinishedLogin:^(id responseObject) {
