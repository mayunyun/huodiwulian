...
#import "YGOptionPicker1.h"

@interface YGOptionPicker1 ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
@implementation YGOptionPicker1

+ (instancetype)sharedOptionPicker {
    
    static YGOptionPicker1 *optionPicker = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!optionPicker) {
            optionPicker = [YGOptionPicker1 new];
        }
    });
    return optionPicker;
}
...
{
    [[YGOptionPicker1 sharedOptionPicker] showOptionPicker:dataSource determineChoose:determineChoose];
}
- (void)showOptionPicker:(NSArray *)dataSource
         determineChoose:(void(^)(NSInteger index,NSString *title))determineChoose
{
    _determineChoose = determineChoose;
    YGOptionPicker1 *picker = [YGOptionPicker1 sharedOptionPicker];
