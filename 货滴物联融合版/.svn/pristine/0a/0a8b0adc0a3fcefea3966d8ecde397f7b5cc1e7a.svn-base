...
#import "JiFenShopViewController1.h"
#import "IconsCollectionViewCell1.h"
#import "DetailsViewController1.h"
#import "ShopCarViewController1.h"
#import "LoginVC1.h"

@interface JiFenShopViewController1 ()<UICollectionViewDataSource,UICollectionViewDelegate>{
    int _numer;
}
...
@implementation JiFenShopViewController1
...
- (UICollectionView *)IconsCollectionView{
    ...
    if (_IconsCollectionView == nil) {
        ...
        //注册单元格
        [_IconsCollectionView registerNib:[UINib nibWithNibName:@"IconsCollectionViewCell1" bundle:nil] forCellWithReuseIdentifier:@"IconsCollectionViewCell1ID"];
    ...
    [[RequestTool1 sharedRequestTool].UserRequestTool1 loginWithUrl:URLStr Parameters:params FinishedLogin:^(id responseObject) {
        ...
        if (Array.count) {
            ...
            for (NSDictionary *diction in Array) {
                //建立模型
                ShopAddrModel1 *model=[[ShopAddrModel1 alloc]init];
...
{
    static NSString *HomeSelarCellID = @"IconsCollectionViewCell1ID";
    
    //在这里注册自定义的XIBcell 否则会提示找不到标示符指定的cell
    UINib *nib = [UINib nibWithNibName:@"IconsCollectionViewCell1" bundle: [NSBundle mainBundle]];
    [_IconsCollectionView registerNib:nib forCellWithReuseIdentifier:HomeSelarCellID];
    
    IconsCollectionViewCell1*cell = [collectionView dequeueReusableCellWithReuseIdentifier:HomeSelarCellID forIndexPath:indexPath];
    cell.backgroundColor = [UIColor whiteColor];
    cell.layer.borderColor = UIColorFromRGBValueValue(0x666666).CGColor;//设置边框颜色
    cell.layer.borderWidth = 1.0f;//设置边框
    if (self.dataArr.count) {
        ShopAddrModel1 *model = self.dataArr[indexPath.section][indexPath.row];
        [cell setData:model];
    }
    return cell;
}
...
//            ShopAddrModel1 *model = self.dataArr[indexPath.section][0];
...
- (void)rightToLastViewController{
    [Command isloginRequest:^(bool str) {
        if (str) {
            ShopCarViewController1 * vc = [[ShopCarViewController1 alloc]init];
