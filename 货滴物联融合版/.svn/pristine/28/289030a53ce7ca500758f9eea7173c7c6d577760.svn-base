...
@property(nonatomic,strong)NSMutableArray *monthdataB;//biaoji
...
-(void)setup{
    ...
    self.height_sd = self.collectionView.bottom_sd;
...
-(void)responData{
    ...
    NSLog(@"%@",self.monthdataA);
    
    NSString *XWURLStr = @"/mbtwz/signinintegral?action=selectSigninByMonth";
    NSDictionary* params = @{@"params":[NSString stringWithFormat:@"{\"yearmonth\":\"%ld%ld\"}",monthModel.year,monthModel.month]};
    ...
    [[RequestTool sharedRequestTool].userRequestTool loginWithUrl:XWURLStr Parameters:params FinishedLogin:^(id responseObject) {
        ...
        if ([[dic objectForKey:@"flag"] intValue]==200) {
            ...
            for (NSDictionary *day in [dic objectForKey:@"response"]) {
                NSString *str1 = [[day objectForKey:@"create_time"] substringFromIndex:8];
                NSString *str2 = [str1 substringToIndex:str1.length-2];
                [dayarr addObject:str2];
            }
            ...
            [self.monthdataA enumerateObjectsUsingBlock:^(LXCalendarDayModel * obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                for (NSString *day in dayarr) {
                    if (obj.day==[day intValue]) {
                        obj.isSelected = YES;
...
-(void)layoutSubviews{
    ...
    self.calendarHeader.frame = CGRectMake(0, 0, self.width, 45);
}
#pragma mark---懒加载
-(LXCalendarHearder *)calendarHeader{
    if (!_calendarHeader) {
        _calendarHeader =[LXCalendarHearder showView];
        _calendarHeader.frame = CGRectMake(0, 0, self.width, 45);
        _calendarHeader.backgroundColor =UIColorFromRGB(0xF8F8F8);
    }
    return _calendarHeader;
}
-(LXCalendarWeekView *)calendarWeekView{
    if (!_calendarWeekView) {
        _calendarWeekView =[[LXCalendarWeekView alloc]initWithFrame:CGRectMake(0, self.calendarHeader.bottom, self.width, 45)];
...
-(UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        flow.itemSize = CGSizeMake(self.width/7, 40*MYWIDTH);
        _collectionView =[[UICollectionView alloc]initWithFrame:CGRectMake(0, self.calendarWeekView.bottom, self.width, 6 * 40*MYWIDTH) collectionViewLayout:flow];
