...
-(void)setModel:(LXCalendarDayModel *)model{
    ...
    self.label.backgroundColor = UIColorFromRGB(0xCACACA);
    
    if (model.isNextMonth || model.isLastMonth) {
        self.userInteractionEnabled = NO;
       self.label.backgroundColor = UIColorFromRGB(0xececec);
        ...
        self.label.textColor = model.currentMonthTitleColor?model.currentMonthTitleColor:UIColorFromRGB(0xFFFFFF);
        if (model.isToday) {
            self.label.textColor = model.todayTitleColor?model.todayTitleColor:[UIColor whiteColor];
        }
    }
