...
#import "PLTagView1.h"
@interface WuliuOrderDetail11 ()<UIScrollViewDelegate,AMapSearchDelegate,CDZStarsControlDelegate,UITextViewDelegate,PLTagViewDelegate, PLTagViewDataSource>
...
@implementation WuliuOrderDetail11
{
    UIView * starView;
...
{
    //
    NSString *URLStr = @"/mbtwz/logisticssendwz?action=searchSuyunOrderDetail";
    ...
    [[RequestTool1 sharedRequestTool].UserRequestTool1 loginWithUrl:URLStr Parameters:params FinishedLogin:^(id responseObject) {
        ...
        if (arr.count) {
            ...
            NSString *URLStr1 = @"/mbtwz/logisticssendwz?action=selectSuyunOrderDetail";
            NSDictionary* params1 = @{@"params":[NSString stringWithFormat:@"{\"id\":\"%@\"}",[_Dictionary objectForKey:@"id"]]};
            [[RequestTool1 sharedRequestTool].UserRequestTool1 loginWithUrl:URLStr1 Parameters:params1 FinishedLogin:^(id responseObject) {
...
-(void)CommitBtnClicked:(UIButton *)sender{
    ...
    if ([_inputTV.text isEqualToString:@""]) {//默认不填任何信息是好评
        dic =@{@"order_id":[NSString stringWithFormat:@"%@",[_Dictionary objectForKey:@"id"]],@"driver_fraction":starScore,@"note":@"默认好评",@"driverid":[NSString stringWithFormat:@"%@",[_Dictionary objectForKey:@"owner_custid"]],@"ordertype":@"1",@"evalute_type":@"1",@"selevalueids":str1};
    }else{
        dic =@{@"order_id":[NSString stringWithFormat:@"%@",[_Dictionary objectForKey:@"id"]],@"driver_fraction":starScore,@"note":_inputTV.text,@"driverid":[NSString stringWithFormat:@"%@",[_Dictionary objectForKey:@"owner_custid"]],@"ordertype":@"1",@"evalute_type":@"1",@"selevalueids":str1};
    }
