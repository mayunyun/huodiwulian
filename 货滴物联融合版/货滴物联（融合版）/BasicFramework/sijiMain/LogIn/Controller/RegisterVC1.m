...
@interface RegisterVC1 ()<UITextFieldDelegate>
{
...
- (void)initSubViewUI{
    ...
    UILabel *other = [[UILabel alloc]initWithFrame:CGRectMake(userView.left, pwdView.bottom, userView.width, 25*MYWIDTH)];
    ...
    UIImageView* pwdView1 = [[UIImageView alloc]initWithFrame:CGRectMake(userView.left, pwdView.bottom+30*MYWIDTH, userView.width, userView.height)];
    ...
    UIButton *nextBut = [[UIButton alloc]initWithFrame:CGRectMake(userView.left, pwdView1.bottom+(self.view.bottom-pwdView1.bottom-userView.height)/2-20*MYWIDTH, userView.width, userView.height)];
...
- (void)nextButClick{
    if (!IsEmptyValue(_userField.text)) {
        if ([Command isPassword:_pwdField.text]) {
            if ([_pwdField.text isEqualToString:_pwdField1.text]&&!IsEmptyValue(_pwdField.text)) {
                [self registRequest];
...
- (void)registRequest{
    ...
    NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"custphone\":\"%@\",\"custname\":\"%@\",\"password\":\"%@\"}",_phoneStr,_userField.text,_pwdField.text]};
