...
#import "XiaoFeiTableViewCell1.h"
#import "MingXiModel1.h"
@interface MingXiViewController1 ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation MingXiViewController1
...
- (UITableView *)tableview{
    if (_tableview == nil) {
        ...
        [_tableview registerNib:[UINib nibWithNibName:@"XiaoFeiTableViewCell1" bundle:nil] forCellReuseIdentifier:@"XiaoFeiTableViewCell1"];
...
- (void)loadNew
{
    ...
    [[RequestTool1 sharedRequestTool].UserRequestTool1 loginWithUrl:XWURLStr Parameters:nil FinishedLogin:^(id responseObject) {
        ...
        for (NSDictionary*dic in arr ) {
            MingXiModel1 *model=[[MingXiModel1 alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    XiaoFeiTableViewCell1 *cell=[tableView dequeueReusableCellWithIdentifier:@"XiaoFeiTableViewCell1"];
    if (!cell) {
        cell=[[XiaoFeiTableViewCell1 alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"XiaoFeiTableViewCell1"];
        
    }
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.backgroundColor =[UIColor clearColor];
    if (_dataArr.count) {
        MingXiModel1 *model = _dataArr[indexPath.row];
        [cell setData:model];
    }
    return cell;
