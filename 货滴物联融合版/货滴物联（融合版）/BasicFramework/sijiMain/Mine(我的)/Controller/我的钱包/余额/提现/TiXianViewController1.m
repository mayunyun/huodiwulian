...
@interface TiXianViewController1 ()<UITextFieldDelegate,UITextViewDelegate>
{
...
- (void)tixianButClicked{
    ...
    NSLog(@"%@",parmas);
    [[RequestTool1 sharedRequestTool].UserRequestTool1 loginWithUrl:URLStr Parameters:parmas FinishedLogin:^(id responseObject) {
        NSString *str = [[NSString alloc]initWithData:responseObject encoding:NSUTF8StringEncoding];
        NSLog(@"我的>>>>%@",str);
        if ([str rangeOfString:@"true"].location!=NSNotFound) {
            //字符串中有true串
            jxt_showToastTitle(@"提现申请成功", 1);
            TiXianRecordViewController1 *tixianJL = [[TiXianRecordViewController1 alloc]init];
            tixianJL.pushNum = 1;
