...
#import "ReplacePayPswVC1.h"
#import "BindEmailViewController1.h"
#import "QQandWXViewController1.h"
@interface SaveCenterViewController1 ()
{
    NSArray* _titleArr;
}
@end

@implementation SaveCenterViewController1
...
- (void)cellClick:(UIButton*)sender{
    if (sender.tag == 100) {
        ...
        //修改密码
        ReplacePwdViewController1* vc = [[ReplacePwdViewController1 alloc]init];
        [self.navigationController pushViewController:vc animated:YES];
    }else if (sender.tag == 101){
        //绑定邮箱
        BindEmailViewController1* vc = [[BindEmailViewController1 alloc]init];
        [self.navigationController pushViewController:vc animated:YES];
        
    }else if (sender.tag == 102){
        //绑定球球、微信
        QQandWXViewController1* vc = [[QQandWXViewController1 alloc]init];
        [self.navigationController pushViewController:vc animated:YES];
    }else if (sender.tag == 103){
        //修改支付密码
        ReplacePayPswVC1* vc = [[ReplacePayPswVC1 alloc]init];
        [self.navigationController pushViewController:vc animated:YES];
    }
