...
#import "TiBanWuLiuSjrzIngViewCell.h"
#import "WuLiuSJRZPhoneCell.h"
#import "TiBanWuLiuSJRZViewController.h"
#import "WuLiuSJRZViewController1.h"

@interface TiBanWuLiuSjrzIngViewController ()<UITableViewDataSource,UITableViewDelegate,UIActionSheetDelegate>
...
@implementation TiBanWuLiuSjrzIngViewController
...
- (void)foodButClick{
    jxt_showAlertTwoButton(@"提示", @"您确定需要重新提交审核吗？", @"确定", ^(NSInteger buttonIndex) {
        TiBanWuLiuSJRZViewController *sjrz = [[TiBanWuLiuSJRZViewController alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        static NSString * stringCell = @"TiBanWuLiuSjrzIngViewCell";
        TiBanWuLiuSjrzIngViewCell * cell = [tableView dequeueReusableCellWithIdentifier:stringCell];
        if (cell == nil) {
...
- (void)rightBarClick:(UIButton*)sender
{
    if (self.status==0) {
        ...
        WuLiuSJRZViewController1 *tiban = [[WuLiuSJRZViewController1 alloc]init];
        tiban.stopPush = 1;
