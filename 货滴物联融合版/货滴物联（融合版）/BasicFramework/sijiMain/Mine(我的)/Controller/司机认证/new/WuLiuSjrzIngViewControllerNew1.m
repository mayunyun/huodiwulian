...
@interface WuLiuSjrzIngViewControllerNew1 ()<UITableViewDataSource,UITableViewDelegate,UIActionSheetDelegate>
...
@implementation WuLiuSjrzIngViewControllerNew1
...
- (void)foodButClick{
    jxt_showAlertTwoButton(@"提示", @"您确定需要重新提交审核吗？", @"确定", ^(NSInteger buttonIndex) {
        WuLiuSJRZViewControllerNew1 *sjrz = [[WuLiuSJRZViewControllerNew1 alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        static NSString * stringCell = @"WuLiuSjrzIngViewCellNew1";
        WuLiuSjrzIngViewCellNew1 * cell = [tableView dequeueReusableCellWithIdentifier:stringCell];
        if (cell == nil) {
        ...
        if (self.imageArr.count) {
            ...
            for (NSDictionary *dic in self.imageArr) {
                NSString *image = [NSString stringWithFormat:@"%@/%@/%@",_Environment_Domain,[dic objectForKey:@"folder"],[dic objectForKey:@"autoname"]];
                 if ([[dic objectForKey:@"driver_img_type"] intValue]==5){
                    [cell.threeImage sd_setImageWithURL:[NSURL URLWithString:image] placeholderImage:[UIImage imageNamed:@"tianjiashili"]];
