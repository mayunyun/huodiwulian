...
#import "WuLiuSJRZViewCellNew1.h"
...
@interface WuLiuSJRZViewControllerNew1 ()<UITableViewDataSource,UITableViewDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UITextFieldDelegate>
...
@implementation WuLiuSJRZViewControllerNew1
{
    UIButton *foodBut;
    NSString * yzmCode;
}
...
- (void)viewDidLoad {
    ...
    if (self.stopPush!=1) {
        [self rightBarTitleButtonTarget:self action:@selector(rightBarClick:) text:@"替班司机"];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        static NSString * stringCell = @"WuLiuSJRZViewCellNew1";
        WuLiuSJRZViewCellNew1 * cell = [tableView dequeueReusableCellWithIdentifier:stringCell];
        if (cell == nil) {
        ...
        cell.oneImage.tag = 1102+indexPath.section;//1101//1102
        ...
        cell.twoImage.tag = 1104+indexPath.section;//1103//1104
        [cell.twoImage addGestureRecognizer:tap2];
        cell.twoImage.image = self.imageArr[indexPath.section+3];
        return cell;
        ...
        static NSString * stringCell = @"WuLiuSJRZViewCellNewPart1";
        WuLiuSJRZViewCellNewPart1 * cell = [tableView dequeueReusableCellWithIdentifier:stringCell];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle]loadNibNamed:stringCell owner:nil options:nil]lastObject];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
//    }
    ...
    NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"driver_name\":\"%@\",\"driver_phone\":\"%@\",\"driver_car_number\":\"%@\",\"driver_checkno\":\"%@\",\"license_type\":\"%@\",\"driving_age\":\"%@\"}",[self.dataDic objectForKey:@"driver_name"],[self.dataDic objectForKey:@"driver_phone"],[self.dataDic objectForKey:@"driver_car_number"],[self.dataDic objectForKey:@"driver_checkno"],[self.dataDic objectForKey:@"license_type"],[self.dataDic objectForKey:@"driving_age"]]};
    
    NSLog(@"%@",params);
    
    
    NSString *URLStr = @"/mbtwz/drivercertification?action=insertDriverCertificationInfo";
...
- (void)butHideClick{
    [SMAlert hide:NO];
    WuLiuSjrzIngViewControllerNew1 *sjrz = [[WuLiuSjrzIngViewControllerNew1 alloc]init];
...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    WuLiuSJRZViewCellNew1 *cell = [self.tableview cellForRowAtIndexPath:indexPath];
    ...
    WuLiuSJRZViewCellNewPart1 *cell1 = [self.tableview cellForRowAtIndexPath:indexPath1];
    if (textField==cell1.carPField){
        [self.dataDic setValue:textField.text forKey:@"driver_car_number"];
    }else if (textField == cell1.driving_age) {
        [self.dataDic setValue:textField.text forKey:@"driving_age"];
    }else if (textField == cell1.license_type){
        [self.dataDic setValue:textField.text forKey:@"license_type"];
    }
}
...
{
    TiBanWuLiuSJRZViewControllerNew1 *tiban = [[TiBanWuLiuSJRZViewControllerNew1 alloc]init];
