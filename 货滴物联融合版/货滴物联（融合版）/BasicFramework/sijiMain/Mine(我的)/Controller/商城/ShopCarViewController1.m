...
@interface ShopCarViewController1 ()<UITableViewDelegate,UITableViewDataSource>{
    double _xiaoji;
    int _xiaojiJF;
    UIView * footerView;
...
@implementation ShopCarViewController1
...
-(void)loadData{
        ...
        [[RequestTool1 sharedRequestTool].UserRequestTool1 loginWithUrl:URLStr Parameters:nil FinishedLogin:^(id responseObject) {
            ...
            for (NSMutableDictionary * dic in reArr) {
                ...
                SABookModel1 * model = [[SABookModel1 alloc]init];
...
#pragma mark -- 删除商品
-(void)deleteProductAction:(SABookModel1 *)model{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString * stringCell = @"ShopCarTableViewCell1";
    ShopCarTableViewCell1 * cell = [tableView dequeueReusableCellWithIdentifier:stringCell];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle]loadNibNamed:stringCell owner:nil options:nil]lastObject];
    }
    SABookModel1 * model = self.dataArr[indexPath.row];
    ...
    [cell setSelectBtnClickBlock:^{
        ...
        for (SABookModel1 *model in self.dataArr) {
...
-(void)allselectBtnClicked:(UIButton *)button{
    ...
    {
        SABookModel1 *model = [self.dataArr objectAtIndex:i];
    ...
    NSLog(@"%@",typeArr);
        ...
        if (str) {
            if ([typeArr containsObject:@1]&&[typeArr containsObject:@2]) {
                ...
                int totaljifen = 0;
                NSString *type;
                for (SABookModel1* model in _dataArr) {
                    if (model.selectState)
                    {
