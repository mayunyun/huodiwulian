...
#import "WQLStarView1.h"
#import "DriverRemarkVC1.h"
#import "MinePageVCTableViewCell1.h"
...
#import "AboutMeViewController1.h"
#import "SaveCenterViewController1.h"
#import "DownloadAppViewController1.h"
#import "AnswerViewController1.h"
...
@interface MinePageVC1 ()<UIGestureRecognizerDelegate,UITableViewDataSource,UITableViewDelegate>
{
    UIView *_headimage;
...
@property (nonatomic,strong)WQLStarView1 * xingxingView;
@property (nonatomic,strong)UIView * starV;
@end

@implementation MinePageVC1
...
-(void)editInfoClicked{
    ...
    EditInfoPageVC1 * vc = [[EditInfoPageVC1 alloc]init];
...
- (UITableView *)tableview{
    if (_tableview == nil) {
        ...
        [_tableview registerNib:[UINib nibWithNibName:@"MinePageVCTableViewCell1" bundle:nil] forCellReuseIdentifier:@"MinePageVCTableViewCell1"];
    }
    return _tableview;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MinePageVCTableViewCell1 *cell=[tableView dequeueReusableCellWithIdentifier:@"MinePageVCTableViewCell1"];
    if (!cell) {
        cell=[[MinePageVCTableViewCell1 alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MinePageVCTableViewCell1"];
        
    }
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    NSArray * arr1 = @[@[@"余额"],@[@"订单"],@[@"司机认证",@"联系客服",@"安全中心",@"用户指南"]];
    NSArray * arr2 = @[@[@"优惠券"],@[@"账单"],@[@"货滴车市",@"关于我们",@"应用下载",@"常见问题"]];
...
- (void)zhangdanClick{
    [Command isloginRequest:^(bool str) {
        if (str) {
            MingXiViewController1 *regiVc = [[MingXiViewController1 alloc]init];
...
- (void)shangchengClick{
    [Command isloginRequest:^(bool str) {
        if (str) {
            NewShopingClassViewController1 *regiVc = [[NewShopingClassViewController1 alloc]init];
            regiVc.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:regiVc animated:YES];
        }else{
            jxt_showAlertTwoButton(@"您目前还没有登录", @"是否前往登录", @"取消", ^(NSInteger buttonIndex) {
    ...
    [Command isloginRequest:^(bool str) {
        if (str) {
            switch (but.tag) {
                case 110:{
                    [[RequestTool sharedRequestTool].userRequestTool loginWithUrl:@"/mbtwz/drivercertification?action=checkDriverSPStatus" Parameters:nil FinishedLogin:^(id responseObject) {
                        ...
                        if (str.length<10) {
                            ...
                            if ([[Arr[0] objectForKey:@"drivertype"] intValue]==1) {
                                TiBanWuLiuSjrzIngViewControllerNew1*ZHVC = [[TiBanWuLiuSjrzIngViewControllerNew1 alloc]init];
                ...
                case 112:{
                    SaveCenterViewController1 *saveVC = [[SaveCenterViewController1 alloc]init];
...
- (void)but2Click:(UIButton *)but{
    [Command isloginRequest:^(bool str) {
        if (str) {
            switch (but.tag) {
                case 210:{
                    CarshopViewController1 *HuodongVC = [[CarshopViewController1 alloc]init];
                    HuodongVC.hidesBottomBarWhenPushed = YES;
                    [self.navigationController pushViewController:HuodongVC animated:YES];
                    break;
                }
                case 211:{
                    AboutMeViewController1 *about = [[AboutMeViewController1 alloc]init];
    ...
    [Command isloginRequest:^(bool str) {
        if (str) {
            switch (but.tag) {
                ...
                case 312:{
                    AnswerViewController1* vc = [[AnswerViewController1 alloc]init];
                    vc.hidesBottomBarWhenPushed = YES;
                    [self.navigationController pushViewController:vc animated:YES];
                    break;
                }
...
- (void)initSubViewUI{
    if (IPHONE_X_Group) {
        _headimage = [[UIView alloc]initWithFrame:CGRectMake(0, statusbarHeight, UIScreenW, 160*MYWIDTH)];
    }else{
    ...
    if (IPHONE_X_Group) {
        _headview = [[UIImageView alloc]initWithFrame:CGRectMake(15*MYWIDTH, 38*MYWIDTH, 70*MYWIDTH, 70*MYWIDTH)];
    }else{
    ...
    _xingxingView = [[WQLStarView1 alloc]initWithFrame:CGRectMake(levImage.right+7*MYWIDTH,1*MYWIDTH, 80*MYWIDTH,20*MYWIDTH) withTotalStar:5 withTotalPoint:5 starSpace:2*MYWIDTH];
    ...
    if (IPHONE_X_Group) {
        qiandao.frame = CGRectMake(UIScreenW-70*MYWIDTH, 38*MYWIDTH, 70*MYWIDTH, 70*MYWIDTH);
    }else{
        qiandao.frame = CGRectMake(UIScreenW-70*MYWIDTH, 18*MYWIDTH, 70*MYWIDTH, 70*MYWIDTH);
    }
    ...
    UIButton *qianbao = [[UIButton alloc]initWithFrame:CGRectMake(0, jianxi*MYWIDTH, 60*MYWIDTH, 70*MYWIDTH)];
    ...
    UIButton *wuliu = [[UIButton alloc]initWithFrame:CGRectMake(0, qianbao.bottom+jianxi*MYWIDTH, 60*MYWIDTH, 90*MYWIDTH)];
    ...
    UIButton *gongju = [[UIButton alloc]initWithFrame:CGRectMake(0, wuliu.bottom+jianxi*MYWIDTH, 60*MYWIDTH, 260*MYWIDTH)];
...
- (void)getLoadDataBasehuoyunTK:(NSNotification *)notice{
    ...
    if (![[[Command getCurrentVC] class] isEqual:[MinePageVC1 class]]) {
        return;
    }
    [Command isloginRequest:^(bool str) {
        if (str) {
...
- (void)starScore{
    DriverRemarkVC1 * vc = [[DriverRemarkVC1 alloc]init];
    vc.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)tuichuClick{
    ...
    [JXTAlertView showAlertViewWithTitle:@"确认退出登录" message:nil cancelButtonTitle:@"取消"otherButtonTitle:@"确定" cancelButtonBlock:nil otherButtonBlock:^(NSInteger buttonIndex) {
        ...
        [[RequestTool1 sharedRequestTool].UserRequestTool1 loginWithUrl:URLStr Parameters:nil FinishedLogin:^(id responseObject) {
            ...
            UINavigationController *lona = [[UINavigationController alloc]initWithRootViewController:[[LoginVC1 alloc]init]];
