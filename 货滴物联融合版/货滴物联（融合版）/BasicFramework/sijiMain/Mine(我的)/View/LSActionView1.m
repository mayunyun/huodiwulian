...
#import "LSActionView1.h"

static const CGFloat kLSActionView1DefaultButtonSpacing = 26;

@interface LSActionView1()
...
@implementation LSActionView1

#pragma mark - Static

+ (instancetype)sharedActionView
{
    static LSActionView1 *actionView = nil;
    static dispatch_once_t token;
    dispatch_once(&token, ^{
        actionView = [[LSActionView1 alloc] init];
    });
    return actionView;
}
...
- (void)configContainerViewWithCount:(NSInteger)count showTitle:(BOOL)showTitle
{
    ...
    _countPerLine = (width - _containerMargin * 2) / (_buttonIconWidth + kLSActionView1DefaultButtonSpacing);
