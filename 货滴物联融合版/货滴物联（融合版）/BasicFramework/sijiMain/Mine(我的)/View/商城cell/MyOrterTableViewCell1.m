...
@interface MyOrterTableViewCell1 ()<UITableViewDelegate,UITableViewDataSource>
{
    UITableView* _tbView;
}

@end
@implementation MyOrterTableViewCell1

{
    MYYMyOrderModel1 *_model;
}
...
{
    MYYOrterClassTableViewCell1* cell = [tableView dequeueReusableCellWithIdentifier:@"MYYOrterClassTableViewCell1"];
    if (!cell) {
        cell = [[NSBundle mainBundle]loadNibNamed:@"MYYOrterClassTableViewCell1" owner:self options:nil].firstObject;
    }
    
    MYYMyOrderClassModer1* model = _prolistArr[indexPath.row];
