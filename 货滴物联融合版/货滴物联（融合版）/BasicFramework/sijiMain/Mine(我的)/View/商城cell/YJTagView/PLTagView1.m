...
@interface PLTagView1 ()<UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
@property (nonatomic, strong) UICollectionView *collectionView;
@end

@implementation PLTagView1
...
- (void) layoutSubviews
{
    ...
    if (height != 0 && height != self.bounds.size.height) {
        ...
        if ([self.delegate respondsToSelector:@selector(DWTagView1:heightUpdated:)]) {
            [self.delegate PLTagView1:self heightUpdated:height];
        }
    }
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    PLCollectionViewCell1 *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"Cell" forIndexPath:indexPath];
    cell.content = [self.dataSource PLTagView1:self titleForItemAtIndex:indexPath.row];
    if (self.themeColor) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(PLTagView1:didSelectedItemAtIndex:)]) {
        [self.delegate PLTagView1:self didSelectedItemAtIndex:indexPath.row];
    }
}
...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewFlowLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return [PLCollectionViewCell1 getSizeWithContent:[self.dataSource PLTagView1:self titleForItemAtIndex:indexPath.row] maxWidth:_collectionView.frame.size.width customHeight:self.cellHeight numer:self.numer];
}
