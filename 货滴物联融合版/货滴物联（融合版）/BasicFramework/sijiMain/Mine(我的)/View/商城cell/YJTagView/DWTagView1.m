...
#import "DWTagView1.h"
#import "UICollectionViewLeftAlignedLayout1.h"
#import "TagCollectionViewCell1.h"

static CGFloat kDefaultCellHeight = 30;

@interface DWTagView1 ()<UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
@property (nonatomic, strong) UICollectionView *collectionView;
@end

@implementation DWTagView1
...
- (void) layoutSubviews
{
    ...
    UICollectionViewLeftAlignedLayout1 *layout = (UICollectionViewLeftAlignedLayout1 *)_collectionView.collectionViewLayout;
    ...
    if (height != 0 && height != self.bounds.size.height) {
        ...
        self.frame = frame;
        _collectionView.frame = self.bounds;
        if ([self.delegate respondsToSelector:@selector(DWTagView1:heightUpdated:)]) {
            [self.delegate DWTagView1:self heightUpdated:height];
        }
    }
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    TagCollectionViewCell1 *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"Cell" forIndexPath:indexPath];
    cell.content = [self.dataSource DWTagView1:self titleForItemAtIndex:indexPath.row];
    if (self.themeColor) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(DWTagView1:didSelectedItemAtIndex:)]) {
        [self.delegate DWTagView1:self didSelectedItemAtIndex:indexPath.row];
    }
}
...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewFlowLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return [TagCollectionViewCell1 getSizeWithContent:[self.dataSource DWTagView1:self titleForItemAtIndex:indexPath.row] maxWidth:_collectionView.frame.size.width customHeight:self.cellHeight numer:self.numer];
}
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame), CGRectGetHeight(self.frame)) collectionViewLayout:[[UICollectionViewLeftAlignedLayout1 alloc] init]];
        _collectionView.delegate = self;
        _collectionView.dataSource = self;
        _collectionView.backgroundColor = [UIColor clearColor];
        [_collectionView registerNib:[UINib nibWithNibName:@"TagCollectionViewCell1" bundle:nil] forCellWithReuseIdentifier:@"Cell"];
    }
