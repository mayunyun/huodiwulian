...
@interface HomeSuYunXiaoJVC1 ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation HomeSuYunXiaoJVC1
{
    NSInteger _page;
...
- (void)loadData
{
    ...
    //
    NSString *URLStr = @"/mbtwz/find?action=selectSuyunOrderList";
    ...
    [[RequestTool1 sharedRequestTool].UserRequestTool1 loginWithUrl:URLStr Parameters:params FinishedLogin:^(id responseObject) {
        ...
        if ([arr count]) {
            for (NSDictionary *dic in [diction objectForKey:@"rows"]) {
                //建立模型
                ZHXiaoJModel1 *model=[[ZHXiaoJModel1 alloc]init];
...
- (UITableView *)tableview{
    if (_tableview == nil) {
        ...
        [_tableview registerClass:[ZHXiaoJTableViewCell11 class] forCellReuseIdentifier:NSStringFromClass([ZHXiaoJTableViewCell11 class])];
        
        _tableview.rowHeight = UITableViewAutomaticDimension;
        _tableview.estimatedRowHeight = 100*MYWIDTH;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    Class ZuLinCarClass = [ZHXiaoJTableViewCell11 class];
    ZHXiaoJTableViewCell11 *cell = nil;
    ...
    if (_dataArr.count) {
        NSLog(@"%@",_dataArr);
        ZHXiaoJModel1 *model = _dataArr[indexPath.row];
        [cell setwithDataModel:model locationStr:self.locationStr];
    }
