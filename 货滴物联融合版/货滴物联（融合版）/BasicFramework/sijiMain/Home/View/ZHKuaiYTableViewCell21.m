...
@interface ZHKuaiYTableViewCell21()
...
@implementation ZHKuaiYTableViewCell21
...
- (void)setwithDataModel:(ZHKuaiYModel1 *)dataModel
{
    ...
    _beizhuLab.sd_layout
    .leftSpaceToView(_bgview, 1.5*margin)
    .topSpaceToView(_jiaoyiLab, 0.3*margin)
    .rightSpaceToView(_bgview, 1.5*margin)
    .bottomEqualToView(_bgview);
    ...
    if ([_dataModel.weight intValue]==0) {
        if ([_dataModel.volume intValue]==0) {
            [_CarTypeView setText:[NSString stringWithFormat:@"%@%@ %@ %@ %@吨",use_type,chechang,[_dataModel.cartypenames stringByReplacingOccurrencesOfString:@" " withString:@"/"],_dataModel.cargotypenames,_dataModel.ton_weight]];
        }else{
            [_CarTypeView setText:[NSString stringWithFormat:@"%@%@ %@ %@ %@吨/%@件",use_type,chechang,[_dataModel.cartypenames stringByReplacingOccurrencesOfString:@" " withString:@"/"],_dataModel.cargotypenames,_dataModel.ton_weight,_dataModel.volume]];
        }
    }else if ([_dataModel.ton_weight intValue]==0){
        if ([_dataModel.volume intValue]==0) {
        ...
        if ([_dataModel.ton_weight intValue]==0) {
            [_CarTypeView setText:[NSString stringWithFormat:@"%@%@ %@ %@ %@Kg",use_type,chechang,[_dataModel.cartypenames stringByReplacingOccurrencesOfString:@" " withString:@"/"],_dataModel.cargotypenames,_dataModel.weight]];
        }else if ([_dataModel.weight intValue]==0){
            [_CarTypeView setText:[NSString stringWithFormat:@"%@%@ %@ %@ %@吨",use_type,chechang,[_dataModel.cartypenames stringByReplacingOccurrencesOfString:@" " withString:@"/"],_dataModel.cargotypenames,_dataModel.ton_weight]];
        }else{
            [_CarTypeView setText:[NSString stringWithFormat:@"%@%@ %@ %@ %@Kg/%@吨",use_type,chechang,[_dataModel.cartypenames stringByReplacingOccurrencesOfString:@" " withString:@"/"],_dataModel.cargotypenames,_dataModel.weight,_dataModel.ton_weight]];
        }
        
    }else{
        [_CarTypeView setText:[NSString stringWithFormat:@"%@%@ %@ %@ %@Kg/%@吨/%@件",use_type,chechang,[_dataModel.cartypenames stringByReplacingOccurrencesOfString:@" " withString:@"/"],_dataModel.cargotypenames,_dataModel.weight,_dataModel.ton_weight,_dataModel.volume]];
    ...
    [_jiaoyiLab setText:[NSString stringWithFormat:@"已交易%@笔",numer]];
    
    _beizhuLab.text = [NSString stringWithFormat:@"%@",_dataModel.beizhu];
    NSMutableParagraphStyle *paraStyle01 = [[NSMutableParagraphStyle alloc] init];
    CGFloat emptylen = _beizhuLab.font.pointSize * 2;
    paraStyle01.firstLineHeadIndent = emptylen;//首行缩进
    NSAttributedString *attrText = [[NSAttributedString alloc] initWithString:_beizhuLab.text attributes:@{NSParagraphStyleAttributeName:paraStyle01}];
    _beizhuLab.attributedText = attrText;
    
    if (_beizhuLab.text.length>0) {
        _beiView.hidden = NO;
        _beiView.image = [UIImage imageNamed:@"beizhu"];
    }else{
        _beiView.hidden = YES;
    }
}
...
- (UIImageView *)beiView
{
    if(_beiView ==nil)
    {
        _beiView =[[UIImageView alloc]init];
        [_bgview addSubview:_beiView];
    }
    return _beiView;
}
...
- (UILabel *)beizhuLab
{
    if(_beizhuLab ==nil)
    {
        ...
        _beizhuLab.font = [UIFont systemFontOfSize:13*MYWIDTH];
        _beizhuLab.textColor = UIColorFromRGB(0x999999);
        _beizhuLab.numberOfLines = 0;
        //_beizhuLab.textAlignment = NSTextAlignmentRight;
        [_bgview addSubview: _beizhuLab];
    }
    return _beizhuLab;
}
- (UIView *)xianView
