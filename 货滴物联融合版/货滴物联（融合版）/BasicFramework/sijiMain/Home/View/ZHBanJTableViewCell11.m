...
#import "HomeSYBanJDetailsViewController1.h"

@interface ZHBanJTableViewCell11()
...
@implementation ZHBanJTableViewCell11
...
- (void)setwithDataModel:(ZHBanJModel1 *)dataModel locationStr:(CLLocationCoordinate2D)location
{
    ...
    NSString *scity = [NSString stringWithFormat:@"%@",_dataModel.owner_address];
    NSString *ecity = [NSString stringWithFormat:@"%@",_dataModel.owner_send_address];
    if (scity.length>6) {
        scity = [scity substringToIndex:6];
    }
    
    if (ecity.length>6) {
        ecity = [ecity substringToIndex:6];
    }
    
    [_titleView setText:[NSString stringWithFormat:@"%@-%@",scity,ecity]];
    if ([NSString stringWithFormat:@"%@",_dataModel.owner_createtime].length>16) {
        [_timeView setText:[[NSString stringWithFormat:@"%@",_dataModel.owner_createtime] substringToIndex:16]];
    }
    ...
    [_juliLab setText:[NSString stringWithFormat:@"%@  交易%@笔 %@",_dataModel.owner_link_name,numer,_dataModel.beizhu]];
    
    CLLocationDegrees latitude = [_dataModel.latitude floatValue];
    CLLocationDegrees longitude = [_dataModel.longitude floatValue];
...
- (void)phoneViewClick{
    ...
    NSString *phone = [NSString stringWithFormat:@"确定拨打电话：%@？",self.dataModel.owner_link_phone];
    ...
    UIAlertAction *otherAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"tel:%@",self.dataModel.owner_link_phone]]];
    }];
...
- (UILabel *)titleView
{
    if(_titleView ==nil)
    {
        ...
        _titleView.font = [UIFont boldSystemFontOfSize:14*MYWIDTH];
    ...
    [[RequestTool1 sharedRequestTool].UserRequestTool1 loginWithUrl:@"/mbtwz/drivercertification?action=checkDriverSPStatus" Parameters:nil FinishedLogin:^(id responseObject) {
        ...
        if (str.length<10) {
            ...
            if ([[Arr[0] objectForKey:@"driver_info_status"] intValue]==1){//审核通过
                HomeSYBanJDetailsViewController1 *detailsVC = [[HomeSYBanJDetailsViewController1 alloc]init];
