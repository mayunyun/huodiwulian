...
#import "WuLiuSJRZViewCellNew.h"
#import "WuLiuSJRZPhoneCellNew.h"
#import "WuLiuSjrzIngViewControllerNew.h"
@interface WuLiuSJRZViewControllerNew ()<UITableViewDataSource,UITableViewDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UITextFieldDelegate>
...
@implementation WuLiuSJRZViewControllerNew
{
    UIButton *foodBut;
    NSString * yzmCode;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        static NSString * stringCell = @"WuLiuSJRZViewCellNew";
        WuLiuSJRZViewCellNew * cell = [tableView dequeueReusableCellWithIdentifier:stringCell];
        if (cell == nil) {
        ...
        static NSString * stringCell = @"WuLiuSJRZPhoneCellNew";
        WuLiuSJRZPhoneCellNew * cell = [tableView dequeueReusableCellWithIdentifier:stringCell];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle]loadNibNamed:stringCell owner:nil options:nil]lastObject];
        }
...
- (void)foodButClick{
    ...
    //        jxt_showToastTitle(@"请填写您的身份证号", 1);
    //        return;
    //    }
    ...
    NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"custname\":\"%@\",\"custcall\":\"%@\",\"custidcard\":\"%@\"}",[self.dataDic objectForKey:@"driver_name"],[self.dataDic objectForKey:@"driver_phone"],[self.dataDic objectForKey:@"driver_car_number"]]};
    
    NSLog(@"%@",params);
    
    
    NSString *URLStr = @"/mbtwz/logisticssendwz?action=insertOwnerCertification";
    ...
    [[RequestTool sharedRequestTool].userRequestTool loginWithUrl:URLStr Parameters:params FinishedLogin:^(id responseObject) {
        ...
        if ([[dic objectForKey:@"flag"] intValue]==300) {
            
            
            jxt_showAlertOneButton(@"提示", @"基本信息提交失败", @"确定", ^(NSInteger buttonIndex) {
            });
        }else if([[dic objectForKey:@"flag"] integerValue] == 200){
            NSString *str1 = [[dic objectForKey:@"response"][0] objectForKey:@"id"];
            //NSString *str1 = [str substringFromIndex:1];
            //图片标识（1：身份证正面照，2：身份证反面照，3：手持身份证件照）
            if (!IsEmptyValue(imageArr1)&&!IsEmptyValue(str1)&&!IsEmptyValue([nuerStr substringFromIndex:7])) {
                [self requestPortal:nil img:imageArr1 idStr:str1 driver_img_type:[nuerStr substringFromIndex:7]];
            }
...
#pragma mark 上传图片的请求
    ...
    NSString* urlStr = [NSString stringWithFormat:@"%@%@",_Environment_Domain,@"/mbtwz/logisticssendwz?action=insertOCUploadImg"];
    NSDictionary* params = @{@"ownerid":idStr,
...
- (void)butHideClick{
    [SMAlert hide:NO];
    WuLiuSjrzIngViewControllerNew *sjrz = [[WuLiuSjrzIngViewControllerNew alloc]init];
...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    WuLiuSJRZViewCellNew *cell = [self.tableview cellForRowAtIndexPath:indexPath];
    ...
    WuLiuSJRZPhoneCellNew * cell1 = [self.tableview cellForRowAtIndexPath:indexPath1];
    if (textField==cell1.carPField){
