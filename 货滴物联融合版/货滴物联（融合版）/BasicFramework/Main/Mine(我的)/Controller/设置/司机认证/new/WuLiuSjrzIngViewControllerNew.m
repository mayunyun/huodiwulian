...
#import "WuLiuSjrzIngViewCellNew.h"
#import "WuLiuSJRZPhoneCellNew.h"
#import "WuLiuSJRZViewControllerNew.h"
@interface WuLiuSjrzIngViewControllerNew ()<UITableViewDataSource,UITableViewDelegate,UIActionSheetDelegate>
...
@implementation WuLiuSjrzIngViewControllerNew
...
- (void)foodButClick{
    jxt_showAlertTwoButton(@"提示", @"您确定需要重新提交审核吗？", @"确定", ^(NSInteger buttonIndex) {
        WuLiuSJRZViewControllerNew *sjrz = [[WuLiuSJRZViewControllerNew alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        static NSString * stringCell = @"WuLiuSjrzIngViewCellNew";
        WuLiuSjrzIngViewCellNew * cell = [tableView dequeueReusableCellWithIdentifier:stringCell];
        if (cell == nil) {
        ...
        if (IsEmptyValue([self.dataDic objectForKey:@"driver_car_number"])) {
            cell.carPField.text = @"";
        }else{
