...
@interface PinpaiSousuoViewController ()<UITableViewDataSource,UITableViewDelegate,CarHomeHeadViewDelegate,UITextFieldDelegate>
...
@implementation PinpaiSousuoViewController
...
- (UITableView *)tableview{
    if (_tableview == nil) {
        ...
        _carHomeHeadView = [[CarHomeHeadView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, 260*MYWIDTH)];
        _carHomeHeadView.delegate = self;
        
        UIView *bgview = [[UIView alloc]initWithFrame:CGRectMake(0, _carHomeHeadView.bottom-50*MYWIDTH, kScreenWidth, 50*MYWIDTH)];
        bgview.backgroundColor = UIColorFromRGB(0xEEEEEE);
        [_carHomeHeadView addSubview:bgview];
...
- (void)headviewlodaData{
    ...
    [[RequestTool sharedRequestTool].userRequestTool loginWithUrl:URLStr Parameters:params FinishedLogin:^(id responseObject) {
        ...
        if (!IsEmptyValue(Arr)) {
            if ([Arr count] >5) {
                _carHomeHeadView.frame = CGRectMake(0, 0, kScreenWidth, 210*MYWIDTH+50*MYWIDTH);
            }else if ([Arr count]>0){
                _carHomeHeadView.frame = CGRectMake(0, 0, kScreenWidth, 132*MYWIDTH+50*MYWIDTH);
            }else{
                _carHomeHeadView.frame = CGRectMake(0, 0, kScreenWidth, 50*MYWIDTH);
            }
