...
@interface FeiLeiShopViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UISearchBarDelegate>
@property(nonatomic,strong)UICollectionView *collection;
@property (nonatomic) NSInteger selectedIndex;

@end

@implementation FeiLeiShopViewController
{
...
- (void)viewDidLoad {
    ...
    _selectedIndex = _selectedIndexs;
...
- (void)creatUI
{
    ...
    _collection =[[UICollectionView alloc]initWithFrame:CGRectMake(0, statusbarHeight+44+40, UIScreenW, UIScreenH-40-statusbarHeight-44-44*MYWIDTH) collectionViewLayout:layout];
    ...
    [_collection registerNib:[UINib nibWithNibName:@"NewShopCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"NewShopCollectionViewCellID"];
    ...
    NSArray *butArr = @[@"导航-首页",@"fenleihong",@"gouwuhei",@"wodehei"];
...
- (void)tabbarBut:(UIButton*)but{
    switch (but.tag) {
        case 0:
        {
            NewShopingClassViewController *NewShopi = [[NewShopingClassViewController alloc]init];
            [self.navigationController pushViewController:NewShopi animated:YES];
            break;
        }
        case 2:
        {
...
- (void)setButtons{
    ...
    for (NSDictionary *dic in [user objectForKey:@"shopARR"]) {
        [_items addObject:[dic objectForKey:@"name"]];
    }
...
{
    self.selectedIndex = self.selectedIndexs;
    [self selectButtonWithIndex:self.selectedIndexs];
}
...
- (void)setButtonsFrames
{
    ...
    CGFloat underLineW = width - 2*10;
    if (underLine != nil) {
        underLine.frame = CGRectMake(self.selectedIndex*width + 10, height-2,
                                     underLineW, 2);
    }
}
...
- (void)requestData
{
    ...
    params = @{@"page":[NSString stringWithFormat:@"%li",(long)_page],@"rows":@"20",@"params":[NSString stringWithFormat:@"{\"type\":\"2\",\"ordertype\":\"1\",\"prodtype\":\"%@\",\"searchstr\":\"\"}",array[self.selectedIndex]]};
    
    NSString *URLStr = @"/mbtwz/scshop?action=selectMallProdsByParam";
...
{
    static NSString *HomeSelarCellID = @"NewShopCollectionViewCellID";
