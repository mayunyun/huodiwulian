...
@interface WuliuOrderDetail1 ()<UITableViewDataSource,UITableViewDelegate,CDZStarsControlDelegate,UITextViewDelegate,MAMapViewDelegate>
...
@implementation WuliuOrderDetail1
{
    UIView * starView;
...
-(void)requestDetail{
    NSString *url = @"/mbtwz/logisticssendwz?action=searchSuyunOrderDetail";
    NSDictionary * dic =@{@"orderno":self.model.orderno};
...
-(void)CommitBtnClicked:(UIButton *)sender{
    ...
    if ([_inputTV.text isEqualToString:@""]) {//默认不填任何信息是好评
        dic =@{@"order_id":self.model.id,@"driver_fraction":starScore,@"note":@"默认好评",@"driverid":self.model.driver_custid,@"ordertype":@"1",@"evalute_type":@"0"};
    }else{
        dic =@{@"order_id":self.model.id,@"driver_fraction":starScore,@"note":_inputTV.text,@"driverid":self.model.driver_custid,@"ordertype":@"1",@"evalute_type":@"0"};
    }
    NSDictionary* KCparams = @{@"data":[Command dictionaryToJson:dic]};//
    [[RequestTool sharedRequestTool].userRequestTool loginWithUrl:url Parameters:KCparams FinishedLogin:^(id responseObject) {
        ...
        if ([str containsString:@"true"]) {
            ...
            dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
                ...
                TCHXiaoJModel * model = _detailArr[0];
...
//根据订单id查询额外需求
-(void)searchNeedData{
    NSString *url = @"/mbtwz/logisticssendwz?action=selectSuyunOrderDetail";
    ...
    [[RequestTool sharedRequestTool].userRequestTool loginWithUrl:url Parameters:KCparams FinishedLogin:^(id responseObject) {
        ...
        if (_detailArr.count) {
            TCHXiaoJModel * model;
            model = _detailArr[0];
            if (![[NSString stringWithFormat:@"%@",model.remarks] isEqualToString:@""]) {
...
-(void)startOrder{
    NSString *url = @"/mbtwz/logisticssendwz?action=startSuyunOrder";
    NSDictionary * dic =@{@"orderno":self.model.orderno};
    NSDictionary* KCparams = @{@"data":[Command dictionaryToJson:dic]};//
    [[RequestTool sharedRequestTool].userRequestTool loginWithUrl:url Parameters:KCparams FinishedLogin:^(id responseObject) {
        NSString * str = [[NSString alloc]initWithData:responseObject encoding:NSUTF8StringEncoding];
...
-(void)endingOrderData{
    NSString *url = @"/mbtwz/logisticssendwz?action=finishSuyunOrder";
    TCHXiaoJModel * model = _detailArr[0];
    NSDictionary * dic =@{@"orderno":self.model.orderno,
                          @"order_money":model.ordertotalprice,
                          @"owner_createtime":model.createtime,
                          @"siji_findtime":model.pickuptime,
...
-(void)DriverSyData{
    ...
    NSDictionary * dic =@{@"orderno":self.model.orderno,@"owner_createtime":self.model.createtime,@"siji_findtime":model.pickuptime,@"order_money":self.model.ordertotalprice,@"driverid":model.driver_custid};
    NSDictionary* KCparams = @{@"data":[Command dictionaryToJson:dic]};//
    [[RequestTool sharedRequestTool].userRequestTool loginWithUrl:url Parameters:KCparams FinishedLogin:^(id responseObject) {
        NSString * str = [[NSString alloc]initWithData:responseObject encoding:NSUTF8StringEncoding];
        NSLog(@"司机失约%@",str);
...
-(void)cancelOrderData{
    NSString *url = @"/mbtwz/logisticssendwz?action=cancelSuyunOrder";
    NSDictionary * dic =@{@"orderno":self.model.orderno,@"owner_createtime":self.model.createtime,@"siji_findtime":self.model.pickuptime,@"order_money":self.model.ordertotalprice};
    NSDictionary* KCparams = @{@"data":[Command dictionaryToJson:dic]};//
    [[RequestTool sharedRequestTool].userRequestTool loginWithUrl:url Parameters:KCparams FinishedLogin:^(id responseObject) {
        NSString * str = [[NSString alloc]initWithData:responseObject encoding:NSUTF8StringEncoding];
        NSLog(@"取消订单%@",str);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        static NSString * stringCell = @"WuliuOrderDetailHeaderCell";
        WuliuOrderDetailHeaderCell * cell = [tableView dequeueReusableCellWithIdentifier:stringCell];
        if (cell == nil) {
        ...
        if ([_needArray[indexPath.row] isEqualToString:@"0"]) {
            ...
            TCHXiaoJModel * model;
            model = _detailArr[0];
            cell.contentLabel.text = [NSString stringWithFormat:@"%@",model.remarks];
        }
        else{
        ...
        cell.price.text = [NSString stringWithFormat:@"%.2f",[self.model.ordertotalprice floatValue]];
...
- (void)zhifuButClicked:(UIButton *)button{
    ...
    if (button.tag == 550) {//支付宝
        jxt_showAlertTwoButton(@"提示", @"您确定使用支付宝支付吗", @"取消", ^(NSInteger buttonIndex) {
            ...
            [Paydetail zhiFuBaoname:@"测试" titile:@"测试" price:model.ordertotalprice orderId:[NSString stringWithFormat:@"%@_%@",model.uuid,[user objectForKey:USERID]] notice:@"1"];
        ...
        jxt_showAlertTwoButton(@"提示", @"您确定使用微信支付吗", @"取消", ^(NSInteger buttonIndex) {
            ...
            [Paydetail wxname:@"货滴充值" titile:@"货滴充值" price:model.ordertotalprice orderId:[NSString stringWithFormat:@"%@_%@",model.uuid,[user objectForKey:USERID]] notice:@"1"];
            //jxt_showAlertTitle(@"正在开发中");
...
-(void)sure{
    ...
    if ([jkTextField.text isEqualToString:@""]) {
        ...
        TCHXiaoJModel * model = _detailArr[0];
        NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"ordertype\":\"1\",\"zhifuprice\":\"%@\",\"uuid\":\"%@\",\"paypassword\":\"%@\"}",model.ordertotalprice,model.uuid,jkTextField.text]};
...
-(void)shipperBalancePay_kyunderline:(NSString*)pay_method{
    TCHXiaoJModel * model = _detailArr[0];
    NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"ordertype\":\"1\",\"zhifuprice\":\"%@\",\"uuid\":\"%@\",\"pay_method\":\"%@\"}",model.ordertotalprice,model.uuid,pay_method]};
...
- (void)paySuccess{
    ...
    [SMAlert hideCompletion:^{
        ...
        WuliuOrderDetail *tongVc = [[WuliuOrderDetail alloc]init];
