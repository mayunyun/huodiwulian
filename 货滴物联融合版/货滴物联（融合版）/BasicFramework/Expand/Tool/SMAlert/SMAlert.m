...
- (void)showImage:(UIImage*)image content:(NSString*)content customView:(UIView*)customView confirmButton:(SMButton*)confirmButton cancleButton:(SMButton*)cancleButton {
    ...
    ShowaAtion action = ^{
        ...
        if (strongSelf.touchToHide) {
            //only content or allow touch to hide
            [strongSelf.backgroundView addTarget:strongSelf action:@selector(hideClick) forControlEvents:UIControlEventTouchUpInside];
        }
        ...
        strongSelf.controlView.backgroundColor = strongSelf.controlViewbackgroundColor;
        if (customView) {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        self.controlViewbackgroundColor = [UIColor whiteColor];
...
+ (void)setcontrolViewbackgroundColor:(UIColor*)color {
    if (!color) {
        return;
    }
    [SMAlert sharedView].controlViewbackgroundColor = color;
}
