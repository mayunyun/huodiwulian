                    ...
                    failure:(requestFailure)failure
{
    ...
    [kNetWorkManager POST:url parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        [ProgressHUD hideProgressHUDAfterDelay:1];
        if (success) {
            success(responseObject,nil);
        }else{
            [ProgressHUD showProgressHUDWithMode:ProgressHUDModeOnlyText withText:@"网络请求失败" afterDelay:HUD_DismisTime isTouched:YES inView:kWindow];
            failure(nil);
        }
